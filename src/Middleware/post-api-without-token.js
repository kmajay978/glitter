...
function postWithoutTOKEN(endpoint, body) {
    ...
    return fetch(endpoint, config)
        .then((response)=>{
            if (response.status === 200) {
                return response.text().then(data=>{
                    return isValidJSON(data)?({data: JSON.parse(data), status: response.status}):({data: {message: "Success"}, status: response.status})
                })
            }
            else {
        ...
        .catch(() => ({data:{message: "Internal Server Error"}, status: 500}));
}
